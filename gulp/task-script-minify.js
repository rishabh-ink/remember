...
gulp.task("script:minify", ["script:test"], function () {
  ...
  return gulp.src([
    ...
    // .pipe(javascriptFiles) // Get only the JavaScript files, not the map files,...
    .pipe($.rename({ suffix: ".min" })) // ... rename them...
    // .pipe(javascriptFiles.restore()) // ... restore the map files...
    .pipe(gulp.dest(CFG.DIR.dist)); // ... to the destination directory.
});
